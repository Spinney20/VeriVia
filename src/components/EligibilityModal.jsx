...
export default function ComplexEligibilityModal({
  open,
  onClose,
  onConfirm,
  projectTitle = "Titlu Proiect",
  userName = "NumeUtilizator" // pentru audit log
}) {
  // -----------------------------------------------------
  // HOOKS & STATE
  // -----------------------------------------------------
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const eligCategory = foundProject.categories?.find(
          (cat) => cat.name === "Eligibilitate"
        );
        if (!eligCategory) return;

        setItems(eligCategory.checklist || []);
        setAuditLog([]); // Resetăm log la deschidere (sau îl poți încărca de undeva)
      } catch (error) {
  ...
  const handleAddSubtask = (itemIndex) => {
    ...
    setItems((prevItems) => {
      ...
      // Dacă nu există subTasks, îl inițializăm
      if (!task.subTasks) {
        task.subTasks = [];
      }
      task.subTasks.push({
  ...
  const handleSave = async () => {
    ...
    const categoryIndex =
      updatedDb.projects[projectIndex].categories?.findIndex(
        (c) => c.name === "Eligibilitate"
      );
    if (categoryIndex === -1) {
      console.error("Categoria 'Eligibilitate' nu a fost găsită.");
      return;
    }
  ...
  return (
    <Box
      sx={{
        ...
        pointerEvents: "auto" // pentru a ne asigura că inputurile sunt focusabile
      }}
    >
      <Card
        sx={{
          ...
          <Box sx={{ flex: 1 }}>
            <LinearProgress
                ...
                sx={{
                    height: 8,
                    borderRadius: 1,
                    backgroundColor: allComplete ? "#e0e0e0" : "#ffe5e5", // fundal: gri sau roșu deschis
                    "& .MuiLinearProgress-bar": {
                    backgroundColor: allComplete ? "#2e7d32" : "#d32f2f", // bară: verde sau roșu
            ...
            {filteredItems.map((item, itemIndex) => {
              ...
              return (
                ...
                >
                  {/* PARENT ROW */}
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    {/* Afișăm ÎNTOTDEAUNA săgeata, indiferent de subTasks */}
                    <IconButton
                      size="small"
